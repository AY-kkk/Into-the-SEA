import { shouldUseDb, shouldUseReal } from '@/lib/env';
...
export function getExamInfoProvider(): ExamInfoProvider {
  // DATA_SOURCE=db 时优先真实库；或显式 EXAM_INFO_PROVIDER=real。
  return shouldUseDb() || shouldUseReal('exam')
    ? new RealExamInfoProvider()
    : new MockExamInfoProvider();
}
